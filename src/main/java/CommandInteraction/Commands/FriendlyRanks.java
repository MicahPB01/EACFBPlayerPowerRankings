...
public class FriendlyRanks implements Command {
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        try (Connection conn = Database.getConnection()) {
            ...
            event.reply("An error occurred while retrieving the friendly rankings.").queue();
        }
    }

    private List<PlayerRank> getRankedPlayers(Connection conn) throws SQLException {
        String query = "SELECT p.name AS player_name, p.discord_id, p.Friendly_Points, t.name AS team_name " +
                "FROM players p " +
                "LEFT JOIN teams t ON p.team_id = t.team_id " +
                "ORDER BY p.Friendly_Points DESC";
        ...
        while (rs.next()) {
            ...
            int friendlyPoints = rs.getInt("Friendly_Points");
